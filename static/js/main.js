...
/**
 * Particle Animation System - Optimized for Mobile
 */
function initParticleAnimation() {
    ...
    if (prefersReducedMotion) {
        return; // Skip particle animation for accessibility
    }
    
    // Performance check - disable on very small screens or low-end devices
    const isLowEndDevice = navigator.hardwareConcurrency && navigator.hardwareConcurrency <= 2;
    const isVerySmallScreen = window.innerWidth < 480;
    
    if (isLowEndDevice && isVerySmallScreen) {
        return; // Skip animation on low-end mobile devices
    }
    ...
    if (window.innerWidth < 480) {
        particleCount = 8; // Very few particles on small phones
        animationIntensity = 0.5;
    } else if (window.innerWidth < 768) {
        particleCount = 12; // Fewer particles on mobile
        animationIntensity = 0.7;
    } else if (window.innerWidth < 1024) {
        particleCount = 18; // Medium particles on tablets
        animationIntensity = 0.8;
    } else {
        particleCount = 25; // Full particles on desktop
    ...
    function createParticle(container, index) {
        ...
        const size = Math.random() * (window.innerWidth < 768 ? 4 : 8) + baseSize;
        ...
        // Reduced opacity on mobile for subtlety
        const opacity = window.innerWidth < 768 ? 0.4 : 0.6;
        particle.style.opacity = opacity;
        
        // Use CSS animations instead of JavaScript for better performance
        particle.style.animationDelay = Math.random() * 6 + 's';
        const duration = Math.random() * 4 + (window.innerWidth < 768 ? 8 : 6); // Slower on mobile
    ...
    function animateParticles() {
        ...
        const throttleRate = window.innerWidth < 768 ? 32 : 16; // 30fps on mobile, 60fps on desktop
        
        particles.forEach((particle, index) => {
            // Reduced movement intensity on mobile
            const moveX = (Math.sin(Date.now() * 0.0008 + index) * 15 * animationIntensity);
            const moveY = (Math.cos(Date.now() * 0.001 + index) * 10 * animationIntensity);
    ...
    // Optimize particles on resize with debouncing
    let resizeTimeout;
    window.addEventListener('resize', function() {
        clearTimeout(resizeTimeout);
        resizeTimeout = setTimeout(() => {
            const newParticleCount = getOptimalParticleCount();
